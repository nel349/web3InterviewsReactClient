...
export const MintNFTs = () => {
  ...
  const candyMachineAddress = new PublicKey(
    '5HbSU4qeijXfFfqDz3yGzJVqFh4nHmZQdvqreVZnEaYg'
  );

  useEffect(() => {
    (async () => {
      // const wallet = 
      const connected = await solanaProvider?.wallet?.isConnected();
  ...
  const addListener = async () => {
    ...
    if (candyGuard == null || candyGuard === undefined)
      return;
  ...
  const checkEligibility = async () => {
    ...
    // console.log("Candy machine : ", JSON.stringify(candyMachine));
